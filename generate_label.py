...
def generate_label(import_json_dict):
    if import_json_dict["session"]["previous_tests"][-1]["executed"]:
        ...
        n += 1
        draw.text((0, n * font_size), "G-code name: {}".format(import_json_dict["session"]["previous_tests"][-1]["gcode_path"].split("gcodes")[-1].translate("\\//"), (0, 0, 0), font=font))
        n += 1
        ...
        if import_json_dict["machine"]["temperature_controllers"]["chamber"]["chamber_heatable"]:
            draw.text((0, n * font_size), "Build chamber temperature: {0} degC".format(import_json_dict["settings"]["temperature_chamber"]), (0, 0, 0), font=font)
            n += 1
        if import_json_dict["machine"]["temperature_controllers"]["extruder"]["part_cooling"]: #TODO
            draw.text((0, n * font_size), "Feedstock material dried prior to printing: at {0} degC for {1} min".format(import_json_dict["material"]["drying"]["drying_temperature"],
                                                                                                                       import_json_dict["material"]["drying"]["drying_time"]), (0, 0, 0), font=font)
            n += 1

        draw.text((0, n*font_size), "Nozzle: {} mm {} nozzle".format(import_json_dict["machine"]["temperature_controllers"]["extruder"]["nozzle"]["size_id"],
                                                                     import_json_dict["machine"]["temperature_controllers"]["extruder"]["nozzle"]["type"]), (0, 0, 0), font=font)
        n += 1
        draw.text((0, n*font_size), "Tested parameters: {}".format(import_json_dict["session"]["previous_tests"][-1]["test_name"].replace("_", " ")), (0,0,0), font=font)
        n += 1
        draw.text((0, n*font_size), "Target: {}".format(import_json_dict["session"]["target"].replace("_", " ")), (0, 0, 0), font=font)
        n += 2
        ...
        if import_json_dict["session"]["previous_tests"][-1]["test_name"] == "printing speed" or "retraction distance":
            ...
            for printing_speed_ind in range(len(import_json_dict["session"]["previous_tests"][-1]["tested_parameter_two_values"])):
                ...
                draw.text((0, 2.*printing_speed_ind*square_size+n*font_size), "{} {}".format(parameter_value, import_json_dict["session"]["previous_tests"][-1]["parameter_two_units"]), (0, 0, 0), font=font_small)
