...
def initialize_test():
    if persistence["session"]["test_type"] == "A":
        ...
        if test_info.name == "retraction distance":
            retraction_distance(ts)
        elif test_info.name == "extrusion temperature vs retraction distance":
            retraction_distance(ts)
        elif test_info.name == "retraction-restart distance":
            retraction_restart_distance_vs_coasting_distance(ts)
        elif test_info.name == "bridging extrusion-multiplier vs bridging printing speed":
            bridging_test(ts)
        else:
...
if quiet:
    ...
    if persistence["machine"]["temperature_controllers"]["printbed"]["printbed_heatable"]:
        persistence["settings"]["temperature_printbed"] = machine.temperaturecontrollers.printbed.temperature_printbed_setpoint
    if persistence["machine"]["temperature_controllers"]["chamber"]["chamber_heatable"]:
        persistence["settings"]["temperature_chamber"] = machine.temperaturecontrollers.chamber.temperature_chamber_setpoint
    if persistence["machine"]["temperature_controllers"]["extruder"]["part_cooling"]:
        persistence["settings"]["part_cooling"] = machine.temperaturecontrollers.extruder.part_cooling
