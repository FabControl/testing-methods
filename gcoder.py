...
class GCode(object):
    ...
    def prepend_to_layer(self, commands, layer_idx):
        ...
        # Find start index to append lines and end index to append new indices
    ...
    def _preprocess(self, lines = None, build_layers = False, layer_callback = None):
            ...
            if line.command:
                ...
                if build_layers:
                    ...
                    if line.command == "G0" or line.command == "G1":
                        ...
                        e = line.e if line.e is not None else 0.0
                        total_e_precise += e
                        ...
                        filament_length += e
                        filament_length_e = total_e_precise
        ...
        if build_layers:
            ...
            self.filament_length_e = filament_length_e
...
def main():
    if len(sys.argv) < 2:
        print("usage: {} save_session_file_as.gcode".format(sys.argv[0]))
        return
