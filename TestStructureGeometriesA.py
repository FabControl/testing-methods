...
class TestStructure(object):
    def __init__(self, persistence, buffer):
    ...
    def raft_perimeter(self, values: get_values_A):
        values.g.set_extruder_temperature(self.machine.settings.temperature_extruder_raft,
                                          self.machine.temperaturecontrollers.extruder, immediate=True)
        values.g.set_extruder_temperature(self.machine.settings.temperature_extruder_raft,
                                          self.machine.temperaturecontrollers.extruder)
        ...
        for dummy in range(2):
            for _ in range(100):
                values.g.move(x=0,
                              ...
                              z=0,
                              extrude=True, extrusion_multiplier=1.5, coef_h=values.coef_h_raft,
                              coef_w=values.coef_w_raft)
            for _ in range(100):
    ...
    def print_raft(self, values: get_values_A):
        ...
        values.g.write(
            "; --- print the infill with the density of {0} % ---".format(self.machine.settings.raft_density))
        ...
        for dummy in range(0, int(step_number)):
            values.g.move(x=0,
                          ...
                          extrude=False, extrusion_multiplier=0)
            for _ in range(100):
                values.g.move(x=(-1) ** (dummy + 1) * (
                            values.test_structure_size - values.coef_w_raft * self.machine.temperaturecontrollers.extruder.nozzle.size_id) / 100,
                              y=0,
                              z=0,
                              extrude=True, extrusion_multiplier=values.extrusion_multiplier_raft,
                              coef_h=values.coef_h_raft, coef_w=values.coef_w_raft)
    ...
    def flat_test_parameter_one_vs_parameter_two(self, values: get_values_A):
        ...
        self.wipe(values,
                  length_multiplier=1 if self.machine.temperaturecontrollers.extruder.nozzle.size_id < 0.59 else 0.85)

        self.print_raft(values) if values.raft else self.raft_perimeter(
            values)  # print the raft to support the test structure
        ...
        for current_test_structure in range(values.number_of_test_structures):
            ...
            if values.test_name == "extrusion temperature vs printing speed":
                values.g.travel(
                    x=-values.test_structure_width[current_test_structure] - values.test_structure_separation,
                ...
                output = "G1 F500 E" + "{:.3f}".format(
                    4 * values.temperature_extruder[current_test_structure] / values.temperature_extruder[0]) + \
                         "; extrude " + "{:.3f}".format(
                    4 * values.temperature_extruder[current_test_structure] / values.temperature_extruder[
                        0]) + " mm of material"  # TODO tool
            ...
            for current_substructure in range(values.number_of_substructures):
                ...
                values.g.write(
                    "; --- testing the following printing speed value: {:.3f} mm/s".format(current_printing_speed))
                ...
                for current_layer in range(values.number_of_layers):  # layers
                    for current_line in range(values.number_of_lines):
                        values.g.move(x=0,
                                      ...
                                      z=0,
                                      extrude=True,
                                      extrusion_multiplier=values.extrusion_multiplier[current_test_structure],
                                      coef_h=values.coef_h[current_test_structure],
                                      coef_w=values.coef_w[current_test_structure])
                    ...
                    if current_layer == values.number_of_layers - 1:
                        ...
                        values.g.abs_move(
                            z=values.abs_z[current_test_structure] + (current_layer + 1) * values.track_height[
    ...
    def retraction_restart_distance_vs_coasting_distance(self, values: get_values_A):
        ...
        for current_test_structure in range(values.number_of_test_structures):
            ...
            for current_substructure in range(values.number_of_substructures):
                values.g.write(
                    "; --- testing the following printing speed value: {:.1f} mm/s".format(current_printing_speed))

                coasting_distance = np.linspace(0, values.coasting_distance, values.number_of_lines)

                for current_line in range(values.number_of_lines):
                    values.g.move(x=0,
                                  y=-(values.step_y / (2 * values.number_of_substructures) - coasting_distance[
                                      current_line]),
                                  z=0,
                                  extrude=True,
                                  extrusion_multiplier=values.extrusion_multiplier[current_test_structure],
                    ...
                    values.g.retract(values.retraction_speed, values.retraction_distance[current_test_structure],
                                     values.retraction_restart_distance[current_test_structure])
    ...
    def bridging_test(self, values: get_values_A):
        ...
        step_y = (values.test_structure_size - (values.number_of_substructures + 1) * number_of_perimeters * np.mean(
            values.track_width) / np.sin(np.deg2rad(angle))) // values.number_of_substructures
        ...
        for current_layer in range(values.number_of_layers):
            ...
            for current_substructure in range(values.number_of_substructures):
                ...
                if current_substructure != values.number_of_substructures - 1:
                    values.g.travel(x=0,
                                    y=-step_y - 2 * number_of_perimeters * np.mean(values.track_width) / np.sin(
                                        np.deg2rad(angle)),
                                    z=0,
                                    lift=1,
                                    retraction_speed=values.retraction_speed,
            ...
            values.g.travel(x=0,
                            y=+(values.number_of_substructures - 1) * step_y + (
                                        values.number_of_substructures - 1) * number_of_perimeters * np.mean(
                                values.track_width) / np.sin(np.deg2rad(angle)),
                            z=0,
                            lift=1,
                            retraction_speed=values.retraction_speed,
                            retraction_distance=np.mean(values.retraction_distance))
        ...
        for index_current_speed, current_speed_value in enumerate(values.parameter_two.values):
            values.g.write(
                "; --- testing the following bridging speed value: {:.1f} mm/s ---".format(current_speed_value))
            values.g.feed(current_speed_value)

            for index_current_extrusion_multiplier, current_extrusion_multiplier_value in enumerate(
                    values.extrusion_multiplier_bridging):
                values.g.write(values.comment_all_values_of_constant_parameters)
                ...
                for step in range(int(step_y / np.mean(values.track_width))):
                    values.g.move(
                        x=(-1) ** (step + 1) * 2 * (step_y - step * np.mean(values.track_width)) * step_x / step_y,
                        y=0,
                        z=0,
            ...
            values.g.travel(x=+values.number_of_test_structures * step_x,
                            y=-2 * step_y - number_of_perimeters * np.mean(values.track_width) / np.sin(
                                np.deg2rad(angle)),
                            z=0,
                            lift=1,
                            retraction_speed=values.retraction_speed,
    ...
    def retraction_distance(self, values: get_values_A):
        ...
        values.g.write(values.comment_all_values_of_variable_parameters)
        self.wipe(values,
                  length_multiplier=1 if self.machine.temperaturecontrollers.extruder.nozzle.size_id < 0.59 else 0.85)  # perform wipe of the nozzle
        self.print_raft(values)  # print the raft to support the test structure
        values.g.write("; --- start to print the test structure ---")
        ...
        for current_test_structure in range(values.number_of_test_structures):
            output = str(
                "; --- testing the {0} of {1} {2} ---".format(values.parameter_one.name, values.parameter_one.precision,
                                                              values.parameter_one.units))
            ...
            for current_substructure in range(values.number_of_substructures):  # TODO comments
                ...
                if values.test_number in ("09", "10", "11"):
                    ...
                    if current_substructure == 0:
                        ...
                        values.g.dwell(30000)
                        output = "G1 F500 E" + "{:.3f}".format(
                            1.5 * values.temperature_extruder[current_test_structure] / values.temperature_extruder[
                                0]) + \
                                 "; extrude " + "{:.3f}".format(
                            1.5 * values.temperature_extruder[current_test_structure] / values.temperature_extruder[
                                0]) + " mm of material"
                    ...
                    output = str("; --- testing the {0} of {1} {2} ---".format(values.parameter_two.name,
                                                                               values.parameter_two.precision,
                                                                               values.parameter_two.units))
                ...
                for current_layer in range(0, values.number_of_layers):  # layers
                    for current_line in range(values.number_of_lines):
                        ...
                        values.g.move(x=0,
                                      y=((-1) ** (current_line + 1)) * values.step_y / (
                                                  3 * values.number_of_substructures),
                                      z=0,
                                      extrude=True,
                                      extrusion_multiplier=values.extrusion_multiplier[current_test_structure],
                                      coef_h=values.coef_h[current_test_structure],
                        ...
                        values.g.deretract(current_retraction_speed, current_retraction_distance,
                                           current_speed_printing)
                    ...
                    if current_layer == values.number_of_layers - 1:
                        if current_substructure == values.number_of_substructures - 1:
                            values.g.travel(x=-values.test_structure_separation,
                                            y=+(
                                                        values.number_of_substructures - 1) * values.step_y / values.number_of_substructures,
                                            lift=1)
                            values.g.abs_move(z=+values.abs_z[current_test_structure],
                                              extrude=False, extrusion_multiplier=0)
                        else:
    ...
    def generate_footer(self, values: get_values_A):
        ...
        if values.chamber_heatable:
            custom_footer = custom_footer + values.chamber.gcode_command.format(values.chamber.temperature_min,
                                                                                values.chamber.tool) + "; set the chamber temperature\n"

        if values.printbed_heatable:
            custom_footer = custom_footer + values.printbed.gcode_command.format(values.printbed.temperature_min,
                                                                                 values.printbed.tool) + "; set the print bed temperature\n"

        custom_footer = custom_footer + values.extruder.gcode_command.format(0,
                                                                             values.extruder.tool) + "; set the extruder temperature\n"
